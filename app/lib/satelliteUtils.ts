...
/**
 * IMPORTANT:
 * Rendering pipeline expects numeric hex colors (not strings).
 * Example: 0xff0000 instead of "#ff0000"
 */
...
export function propagateOne(tle: TleRecord, when: Date): SatellitePoint | null {
  try {
    ...
    return {
      ...
      baseColor: inclinationColor(inclination),
    }
  } catch {
    return null
  }
...
export function hardwareBucket(norad: number, name: string): string {
  ...
  if (u.includes('V2 MINI')) {
    return u.includes('D2C') ? 'v2 mini d2c' : 'v2 mini'
  }
